...
Real-data validation: v7.1 data is used as the reference dataset.
The pipeline is version-agnostic (works on parsed CartoMesh objects),
so one real dataset is sufficient for integration smoke tests.
Visual inspection of GLB output remains the primary validation for
version-specific rendering after major changes.
"""
...
# v7.1 — single mesh (1-Map), sparse points
...
class TestRealCartoV71:
    ...
    def test_animated_with_vectors(self, v71_study, v71_output):
        ...
        if not quality.suitable:
            pytest.skip("No meshes suitable for vectors in v7.1 dataset")

        mesh_idx = quality.suitable_indices[0]
        mesh = v71_study.meshes[mesh_idx]
        points = v71_study.points.get(mesh.structure_name)
        ...
        output = v71_output / f"{mesh.structure_name}_lat_animated_vectors.glb"
    ...
    def test_static_all_colorings(self, v71_study, v71_output):
        ...
        for coloring in ["lat", "bipolar", "unipolar"]:
            mesh_data = carto_mesh_to_mesh_data(mesh, points, coloring=coloring)
            output = v71_output / f"{mesh.structure_name}_{coloring}.glb"
            build_glb([mesh_data], output)
            assert output.exists()
